...
function LoginContent() {
  ...
  // Use state with initial value from props/searchParams to avoid setState in effect
  const [error, setError] = useState(
    errorParam === "email_exists" 
      ? "This email is already registered. Please log in with your password, or use reset password." 
      : ""
  );
  ...
  const handleGoogle = () => {
    api.getGoogleAuthUrl().then(res => {
      if (res.url) window.location.href = res.url;
    }).catch(() => setError("Failed to initialize Google login"));
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-black">
      <div className="w-full max-w-md p-8 bg-[#0f0f0f] rounded-xl border border-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              ...
              <Link href="/forgot-password" className="text-xs text-gray-500 hover:text-white">Forgot password?</Link>
            </div>
            <input
              type="password"
              value={password}
        ...
        <button
          ...
          className="w-full flex items-center justify-center space-x-2 bg-black border border-gray-700 text-white font-medium py-2 rounded hover:bg-gray-900 transition"
        >
